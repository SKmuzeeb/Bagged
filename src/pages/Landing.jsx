...
export default function Landing() {
  return (
    <div>
      {/* Hero */}
      <section className="mx-auto max-w-content px-5 pb-16 pt-14 md:px-8 md:pb-24 md:pt-20 lg:px-12">
        <div className="flex flex-col items-center gap-12 lg:flex-row lg:items-center lg:gap-16">
          <div className="w-full lg:w-[58%]">
            <p className="mb-4 text-sm font-medium tracking-wide text-accent">For your neighborhood.</p>
            <h1
              className="font-display text-5xl font-medium leading-[1.1] tracking-tight text-ink sm:text-6xl lg:text-7xl"
              style={{ fontVariationSettings: "'opsz' 96" }}
            >
              Aap ke kirana se, aap ke waqt pe.
            </h1>
            ...
            <div className="mt-9 flex flex-wrap items-center gap-6">
              <Link
                to="/shop"
                className="rounded-full bg-accent px-7 py-3.5 text-sm font-medium text-surface transition-colors hover:bg-accent/90"
              >
                Start ordering
              </Link>
      ...
      <section className="border-y border-border bg-surface py-16 md:py-24">
        <Reveal className="mx-auto grid max-w-content grid-cols-1 gap-10 px-5 sm:grid-cols-3 md:px-8 lg:px-12">
      ...
      <section id="how-it-works" className="scroll-mt-24 py-16 md:py-24">
        <Reveal className="mx-auto max-w-content px-5 md:px-8 lg:px-12">
          <h2 className="mb-12 font-display text-4xl font-medium text-ink">How it works</h2>
          <div className="grid grid-cols-1 gap-10 sm:grid-cols-3 sm:gap-8">
            {STEPS.map((step) => (
              <div key={step.number}>
                <span className="font-display text-5xl font-medium text-accent-soft">{step.number}</span>
      ...
      <section className="py-16 md:py-24">
        <Reveal className="mx-auto max-w-content px-5 md:px-8 lg:px-12">
          <div className="flex flex-col overflow-hidden rounded-2xl border border-border bg-surface md:flex-row">
            <div className="aspect-[4/3] w-full md:aspect-auto md:w-[38%]">
              <img
                src={HERO_IMAGES.shopkeeperPortrait}
                alt={`${KIRANA.owner_name}, owner of ${KIRANA.name}`}
                className="h-full w-full object-cover"
                onError={(event) => {
                  event.currentTarget.style.display = 'none'
                }}
            ...
            <div className="flex flex-1 flex-col justify-center p-8 md:p-12">
              <p className="text-sm font-medium text-accent">Meet your kirana</p>
              <h3 className="mt-2 font-display text-3xl font-medium text-ink">{KIRANA.name}</h3>
              <p className="mt-1 text-ink-soft">{KIRANA.owner_name}</p>
              <p className="mt-4 text-sm leading-relaxed text-ink-soft">{KIRANA.address}</p>
              <p className="mt-3 text-sm italic text-ink-soft">{KIRANA.tagline}</p>
              <p className="mt-4 text-sm font-medium text-ink">7 AM – 10 PM, every day</p>
            </div>
          </div>
        </Reveal>
      </section>
      ...
      <section className="border-t border-border py-20 md:py-28">
        <Reveal>
          <PullQuote>
            The best kirana knows your name. We just help you skip the wait.
          </PullQuote>
        </Reveal>
      </section>
    </div>
